...
describe("readYamlEnvSync()", () => {
  ...
  test("should throw exceptions on incorrect file path", () => {
    expect(() =>
      readYamlEnvSync(getFixtureFile("missed-file.yml"))
  ...
  test("merges two yaml config files", () => {
    const config = readYamlEnvSync([
      getFixtureFile("config.yml"),
      getFixtureFile("flat_config.yml"),
    ]);
    expect(config).toMatchSnapshot();
  });
...
describe(".readYamlEnv() async", () => {
  test("should not fail if specified incorrect encoding", async () => {
    const result = await readYamlEnv(
      getFixtureFile("config.yml"),
      undefined,
      undefined,
      "base64"
  ...
  test("should throw exceptions on incorrect file path", async () => {
    await expect(async () =>
      readYamlEnv(getFixtureFile("missed-file.yml"))
  ...
  test("merges two yaml config files", async () => {
    const config = await readYamlEnv([
      getFixtureFile("config.yml"),
      getFixtureFile("flat_config.yml"),
    ]);
    expect(config).toMatchSnapshot();
  ...
  test("should read config with defaults env values", async () => {
    const config = await readYamlEnv(
      getFixtureFile("config-env-defaults.yaml")
    );
...
test("should parse simple yaml", () => {
  ...
  expect(readYamlEnvSync(getFixtureFile("./simple.yaml"))).toStrictEqual({
    test: "my var",
  });
});
...
test("couple defaults in one yaml field", () => {
  expect(
    readYamlEnvSync(getFixtureFile("./couple-defaults.yaml"))
  ).toStrictEqual({
    mysql: {
